...
int main()
{
  double daphneOri = 100.0;
  double cleoOri = 100.0;
  double daphneRate = 0.10;
  double cleoRate = 0.05;
  double daphneNow;
  double cleoNow;
  int year = 0;
  daphneNow = daphneOri + daphneOri*daphneRate;
  cleoNow = cleoOri+ cleoOri*cleoRate;
  while(daphneNow >= cleoNow)
    {
      ++year;
      daphneNow = daphneNow + daphneOri*daphneRate;
      cleoNow = cleoNow + cleoNow*cleoRate;
      std::cout << "Year: " << year << std::endl;
      std::cout << "--- Daphne: " << daphneNow << std::endl;
      std::cout << "--- Cleo: " << cleoNow << std::endl;
    }
  return 0;
}
